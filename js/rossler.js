...
function Rossler(canvas) {
    ...
    this.display = {
        ...
        draw_heads: false,
        damping: true,
        _length: 4096 // change through length getter/setter
    };
    ...
    this.tail_index_buffer = Rossler.create_index_array(gl, length);
    this.tail_element_buffer = Rossler.create_element_array(gl, length);
    ...
    Rossler.fetch(shaders, function(project, tail_v, tail_f, head_v, head_f) {
        this.programs.tail = Rossler.compile(gl, project + tail_v, tail_f);
        this.programs.head = Rossler.compile(gl, project + head_v, head_f);
 ...
 */
Rossler.fetch = function(urls, callback) {
    var results = [];
 ...
 * @returns {Object}
 */
Rossler.compile = function(gl, vert, frag) {
 ...
 * @returns {WebGLBuffer}
 */
Rossler.create_element_array = function(gl, length) {
 ...
 * @returns {WebGLBuffer}
 */
Rossler.create_index_array = function(gl, length) {
 ...
 * @returns {undefined}
 */
Rossler.rossler = function rossler(s, dt, σ, β, ρ) {
    
    function dx(x, y, z) { return -y - z; }
    function dy(x, y, z) { return x + σ * y; }
    function dz(x, y, z) { return β + z * (x - ρ); }
 ...
 * @param {number} b
 */
Rossler.prototype._update = function(a, b) {
    var gl = this.gl;
    var length = this.display._length;
 ...
 * @returns {Rossler} this
 */
Rossler.prototype.step = function() {
    if (!this.ready)
        return this;
    if (!this.params.paused) {
        ...
        for (var s = 0; s < this.params.steps_per_frame; s++) {
            ...
            for (var i = 0; i < this.solutions.length; i++)  {
                Rossler.rossler(this.solutions[i], dt, σ, β, ρ);
 ...
 * @returns {Rossler} this
 */
Rossler.prototype.draw = function() {
    if (!this.ready)
        return this;
 ...
 */
Rossler.prototype._grow_buffers = function() {
    function next2(x) {
 ...
 * @returns {Rossler} this
 */
Rossler.prototype.add = function(s) {
    var gl = this.gl;
    var length = this.display._length;
 ...
 * @returns {Rossler} this
 */
Rossler.prototype._trim = function(length) {
    ...
    this.tail_index_buffer = Rossler.create_index_array(this.gl, length);
    this.tail_element_buffer = Rossler.create_element_array(this.gl, length);
    this._update(0, length - 1);
    return this;
};
 ...
 * @returns {Rossler} this
 */
Rossler.prototype.empty = function() {
    this.solutions = [];
    this.tail = new Float32Array(0);
    this.tail_index = 0;
    this.tail_colors = new Float32Array(0);
...
Object.defineProperty(Rossler.prototype, 'length', {
    get: function() {
